...
namespace EasyStorage
{
    ...
    public abstract partial class SaveDevice
    {
        ...
        private StorageContainer OpenContainer(string containerName)
        {
            ...
            var asyncResult = storageDevice.BeginOpenContainer(containerName, null, null);
        ...
        {
            if (!IsReady)
                throw new InvalidOperationException("Tároló nem áll készen!");
        }
        ...
        public void Save(string containerName, string fileName, FileAction saveAction)
        {
            ...
            {
                // open a container
                using (var currentContainer = OpenContainer(containerName))
                {
                    // attempt the save
        ...
        public void Delete(string containerName, string fileName)
        {
            ...
            lock (storageDevice)
            {
                ...
                using (var currentContainer = OpenContainer(containerName))
                {
                    ...
                    if (currentContainer.FileExists(fileName)) currentContainer.DeleteFile(fileName);
                }
            }
        }
            ...
            lock (storageDevice)
            {
                ...
                using (var currentContainer = OpenContainer(containerName))
                {
                    return string.IsNullOrEmpty(pattern)
                        ? currentContainer.GetFileNames()
                        : currentContainer.GetFileNames(pattern);
                }
            }
        }
