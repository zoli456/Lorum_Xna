...
namespace Nuclex.Game.Content
{
    ...
    public class LzmaContentManager : ContentManager
    {
        ...
        /// </summary>
        private readonly bool replacementAllowed;
        ...
        {
            if (calledByUser)
                if (lzmaPackageStream != null)
                {
                    lzmaPackageStream.Dispose();
                    lzmaPackageStream = null;
                }
        ...
        protected override Stream OpenStream(string assetName)
        {
            ...
            {
                var xnbAssetFilename = Path.ChangeExtension(assetName, ".xnb");
                var xnbAssetPath = Path.Combine(RootDirectory, xnbAssetFilename);
            ...
            if (files != null) return uncompressFromPackage(assetName);

            // Nope, look for an individually compressed file!
        ...
        private Stream uncompressFromPackage(string assetName)
        {
            ...
            if (!files.TryGetValue(assetName.ToLower(), out fileInfo))
            ...
            lock (lzmaPackageStream)
            {
                lzmaPackageStream.Position = fileInfo.Offset;
                return uncompress(
                    lzmaPackageStream, fileInfo.CompressedLength, fileInfo.UncompressedLength
                );
            }
        }
        ...
        private Stream uncompressIndividualFile(string assetName)
        {
            ...
            var lzmaAssetFilename = Path.ChangeExtension(assetName, ".lzma");
            var lzmaAssetPath = Path.Combine(RootDirectory, lzmaAssetFilename);
            ...
            using (
                var compressedFile = new FileStream(
        ...
        private Stream uncompress(Stream source, int compressedLength, int uncompressedLength)
        {
            ...
            var uncompressedMemory = new MemoryStream(uncompressedLength);
        ...
        private void openPackage(string packagePath)
        {
            ...
            if (replacementAllowed)
                ...
                {
                    files = new Dictionary<string, CompressedFileInfo>();
                    return;
                }

            var lzmaPackageStream = new FileStream(
            ...
            try
            {
                ...
                files = new Dictionary<string, CompressedFileInfo>(fileCount);
                ...
                for (var fileIndex = 0; fileIndex < fileCount; ++fileIndex)
                {
                    ...
                    files.Add(name.ToLower(), fileInfo);
                }
            }
            catch (Exception)
            {
