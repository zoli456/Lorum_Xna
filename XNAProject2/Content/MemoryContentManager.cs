...
namespace Nuclex.Game.Content
{
    ...
    /// </remarks>
        ...
        public AssetType ReadAsset<AssetType>(byte[] content)
        {
            lock (this)
            {
                using (memoryStream = new MemoryStream(content, false))
                {
        ...
        {
            if (memoryStream == null) memoryStream = new MemoryStream(content, false);
    ...
    /// <returns>A string containing the SHA-1 sum of the byte array</returns>
        ...
        {
            if (sha1HashProvider == null)
            {
                sha1HashProvider = SHA1.Create();
            }
            byte[] hashCode = sha1HashProvider.ComputeHash(data);
